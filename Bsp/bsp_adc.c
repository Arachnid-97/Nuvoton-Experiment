...
static uint32_t Read_Bandgap(void)
{
	...
	bandgap_value = 3072 / (0x0fff / bandgap_value);	// 换算成千倍的电压值
...
uint32_t VDD_Read(void)
{
	...
	VDD_value = ADCRL;
	VDD_value |= (ADCRH << 4);
	VDD_value = 0x1000 / VDD_value * Read_Bandgap();		// 换算成千倍的电压值
	ADCCON1 &= ~0x01;			// 关闭 ADC
...
uint32_t ADC_Read( const uint32_t Ref )
{
    ...
    ADC_value = (uint32_t)((float)ADC_value*Ref/ 0x1000 );     // 换算成千倍的电压值
